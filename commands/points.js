    ...
    execute(message, args) {
      ...
      if(args.length==3){
        ...
        fs.readFile(/*'./listes/'+args[0]+*/'listes/'+args[0]+'.txt', 'utf8', function read(err, data) {
          ...
          for(var i=0; i<dataToUse.length; i+=2){
            if(dataToUse[i]===args[1]){
              if(i!==dataToUse.length-1){
                ...
                newPlayer=false;
                fs.appendFile('listes/log.txt', `${msg.author.username}`+` `+`${dataToUse[i]}`+` `+`${newPoints}`+` `+`${oldPoints}`+` `+`${args[2]}`+`\r\n`, function (err) {
                  if (err) {
                    //throw err;
                    return console.error(err);
                  }
          ...
          if(newPlayer){
            dataToUse.push(args[1]);
            dataToUse.push(args[2]);
            msg.channel.send("The new score is "+`${args[1]}`+" for "+`${args[2]}`);
            fs.appendFile('listes/log.txt', `${msg.author.username}`+` `+`${args[1]}`+` `+`${args[2]}`+` `+`(0)`+` `+`${args[2]}`+`\r\n`, function (err) {
              if (err) {
                //throw err;
                return console.error(err);
              }
